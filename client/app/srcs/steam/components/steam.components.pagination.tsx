...
import { SortOrder } from "../serviceV2/hook/steam.hooks.getTable";
...
const Pagenation: React.FC<Props> = ({
  ...
  const totalPage = useMemo(
    () => Math.ceil(totalDatasNum / viewNum),
    [totalDatasNum, viewNum]
  );
  ...
  return (
    <nav aria-label="Page navigation example">
      <ul className="flex items-center -space-x-px h-8 text-sm">
        ...
        {Array.from({
          length: Math.min(
            totalPage - currentView * PAGE_VIEW_COUNT,
            PAGE_VIEW_COUNT
          ),
        }).map((_, i) => (
          <li key={`page_${i}`}>
            <button
              ...
              className={`flex items-center justify-center px-3 h-8 leading-tight text-gray-500 bg-white border border-gray-300 hover:bg-gray-100 hover:text-gray-700 
                ${
                  currentView * PAGE_VIEW_COUNT + i + 1 === current
