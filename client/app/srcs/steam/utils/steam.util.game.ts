...
export const dataFetch = async (gameSet: OwnedGames[]) => {
  ...
  const gameDataFromApi = await Promise.all(
    gameSet.map((game) =>
      fetch(`/steam_api/appdetails?appids=${game.appid}&cc=ko-kr`).then((res) =>
        res.json()
      )
    )
  );
  const gameDataWithTags = (
    await Promise.all(gameSet.map((game) => crawlingDataFromAppid(game)))
  ).filter((game) => game !== undefined);
  gameDataFromApi.forEach((game) => {
    const appid = Object.keys(game)[0];
    if (game[appid].success) {
      const crawlingInfo = gameDataWithTags.find(
        (gameData) => (gameData as CrawlingData).appid === Number(appid)
      );
      gameinfos.push({
        personal_data: gameSet.find(
          (gameData) => gameData.appid === Number(appid)
        ) as OwnedGames,
...
export const makeVectorSet = (gameinfos: RawGameCategory) => {
  ...
  const vectorSet: any[] = [];
  gameinfos.forEach((game) => {
    const vector = Array.from(categories).map((category) =>
      game.categories.includes(category) ? 1 : 0
    );
