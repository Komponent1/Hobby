...
export const getArticle = (article_id: string, navigate: NavigateFunction, location: Location, dep: string) => ({
  type: ARTICLE,
  payload: { article_id, navigate, location, dep }
});
export function *Saga(action: any) {
  const { article_id, navigate, location, dep }:
  { article_id: string, navigate: NavigateFunction, location: Location, dep: string } = action.payload;
